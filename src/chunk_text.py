...
def chunking_text(dialog_chunks):
    ## further split sentences
    dialog_text = " ".join(dialog_chunks)

    tokenizer = nltk.data.load(SENTENCE_TOKENIZER)
    dialog_sentences = tokenizer.tokenize(dialog_text)

    # Split text to sentences and get their embedding
    sentences, embeddings = create_embedding(dialog_sentences)
    # Create a dataframe which returns sentences with generated timestamps
    df = create_df(sentences)
    ...
    for num, each in enumerate(df['sentence']):
        if np.isin(num, true_middle_points):
